...
async function fetchCargos() {
  try {
    ...
    cargos.value = response.cargos
    filteredCargos.value = response.cargos
  } catch (error) {
...
async function submitCargo() {
  ...
  try {
    await $fetch('/api/cargo/register', {
      method: 'POST',
      body: cargoData.value
    })
    
    message.value = 'Cargo registered successfully!'
    clearForm()
    await fetchCargos() // Refresh the list automatically
  } catch (error) {
...
async function deleteCargo(cargo) {
  ...
  try {
    ...
    await fetchCargos() // Refresh the list
